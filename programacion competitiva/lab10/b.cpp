...
ll   mini(int v,int tl,int tr,int l,int r){
    /// sum of elements in [l,r] contained(if valid) in vertex v:[tl,tr]
    if(l>r) return inf; // tbm usar inf
    if(l==tl && r==tr) return t[v];
    int tm= tl+(tr-tl)/2;

    ll s1=mini(2*v  ,tl  ,tm,l           ,min(r,tm)); /// intersection [l,r] and [l   ,tm]
    ll s2=mini(2*v+1,tm+1,tr,max(tm+1,l) ,r        ); /// intersection [l,r] and [tm+1, r]
