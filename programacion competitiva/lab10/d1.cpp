...
int find_min_index(int v, int tl, int tr, int l, ll x) {
    if (tl == tr) {
        if (tl >= l && t[v] >= x)
            return tl;
        else
    ...
    int tm = tl + (tr - tl) / 2;
    if (tm >= l && t[2 * v] >= x)
        return find_min_index(2 * v, tl, tm, l, x);
    else if (t[2 * v + 1] >= x)
        return find_min_index(2 * v + 1, tm + 1, tr, l, x);
    else
        return -1;
}
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (op == 1) {
            ...
            ll x;
            cin >> x >> l;
            int index = find_min_index(1, 0, n - 1, l, x);
            cout << index << "\n";
        }
    }
